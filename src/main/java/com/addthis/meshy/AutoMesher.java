...
class AutoMesher implements Runnable {
    ...
    public AutoMesher(MeshyServer meshyServer, int port, int timeout) {
    ...
    private DatagramSocket newSocket() throws SocketException {
        return new DatagramSocket(meshyServer.serverPort);
    }
        ...
        try (final DatagramSocket server = newSocket()) {
            ...
            while (true) {
                ...
                if (time - lastTransmit > timeout) {
                    MeshyServer.log.debug("{} AutoMesh.xmit {} members",
                            meshyServer, meshyServer.group.getMembers().length);
                    server.send(encode());
                    lastTransmit = time;
                }
                try {
    ...
    private DatagramPacket encode() throws IOException {
        ...
        MeshyServer members[] = meshyServer.group.getMembers();
